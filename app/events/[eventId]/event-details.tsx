...
import { CalendarActions } from "../components/calendar-actions/calendar-actions";
...
function eventBlockForCalendar(event: Event, referenceDate: string) {
	const blocks = sortedEventBlocks(event);
	const seed = blocks.find((block) => !block.recurrence_date);

	if (!event.recurrence_rule) {
		const firstBlock = blocks[0];
		const lastBlock = [...blocks].sort(
			(left, right) => right.ends_at.valueOf() - left.ends_at.valueOf(),
		)[0];

		return firstBlock && lastBlock
			? { ...firstBlock, ends_at: lastBlock.ends_at }
			: undefined;
	}

	const nextStart = seed
	...
	const nextBaseBlock =
		seed && nextStart
			...
			: null;
	const upcomingOverrides = blocks.filter(
		(block) =>
			block.recurrence_date &&
			formatDateKey(block.starts_at, block.timezone) >= referenceDate,
	);

	return [...(nextBaseBlock ? [nextBaseBlock] : []), ...upcomingOverrides].sort(
		(left, right) => left.starts_at.valueOf() - right.starts_at.valueOf(),
	)[0];
}
...
export function EventDetails({ event, referenceDate }: EventDetailsProps) {
	const blocks = sortedEventBlocks(event);
	const primaryBlock = primaryEventBlock(event);
	const calendarBlock = eventBlockForCalendar(event, referenceDate);
	const overviewBlock = calendarBlock ?? primaryBlock;
	const recurrenceSummary = eventRecurrenceSummary(event, primaryBlock);
	const primaryLocationDescription =
		overviewBlock?.location_description || event.location_description;
	const primaryLocationAddress =
		overviewBlock?.location_address ||
		(primaryLocationDescription === event.location_description
					...
					<div className={style.detailsCard}>
						...
						<dl className={style.detailsList}>
							<div>
								...
								<dd>
									{overviewBlock ? (
										<time dateTime={overviewBlock.starts_at.toISOString()}>
											{formatBlockDate(overviewBlock)}
										</time>
									) : (
										"Not available"
									)}
						...
						{calendarBlock && (
							<div className={style.calendarAction}>
								<CalendarActions
									calendarEvent={toCalendarEvent(event, calendarBlock)}
									fullWidth
								/>
							</div>
						)}
